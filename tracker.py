...
def obtener_idiomas_de_m3u8(m3u8_url):
    try:
        ...
        m3u8_master = m3u8parser(res.text)
        master_base_url = '/'.join(m3u8_url.split('/')[:-1]) + '/'
        
        audios, subtitulos = set(), set()
        
        # Parsear las pistas multimedia del HLS maestro tal como lo hacía el script original
        for media in m3u8_master.get('media', []):
            ...
            if media['type'] == 'AUDIO':
                ...
                is_forced = media.get('forced') == 'YES'
                sub_tag = f"{lang_clean}-forced" if is_forced else lang_clean
...
def obtener_hls_por_id(episode_id):
    """Consulta el endpoint de contenido individual para extraer el hlsUrl real"""
    url = f"https://tv.apple.com/api/uts/v3/channel/content?id={episode_id}"
    try:
        ...
        for x in resp_content.get('playables', []):
    ...
    try:
        while True:
            ...
            for ep in episodes_list:
                ...
                # Obtenemos la URL del manifiesto HLS usando el ID del episodio
                hls_url = obtener_hls_por_id(ep_id)
...
def main():
    archivo = 'datos.json'
    print("Iniciando escaneo con lectura de manifiestos HLS...")
    nuevos_datos = escanear_episodios()
    
    if nuevos_datos:
        with open(archivo, 'w', encoding='utf-8') as f:
            json.dump(nuevos_datos, f, indent=4, ensure_ascii=False)
        print(f"Escaneo completado. Total de episodios: {len(nuevos_datos)}")
    else:
